...
unsigned long BC_delay(unsigned char state, int weight,unsigned long time ); // the delay for chicken and beef
unsigned long A_delay(unsigned long time); //determine the delay fot popcorn
unsigned long D_delay (unsigned long time, int seconds, int minutes);//A function to determine D(custom) delay
